...
## 現在のステータス（2026-07-04 時点）

| 項目 | 状態 |
|------|------|
| 最終フェーズ | **Phase 55（シグナル売り抑制：含み損時のシグナル売りを停止）** |
| フロントエンド | Vercel稼働中（kabu-signal-navi.vercel.app） |
| バックエンド | Render稼働中（stock-signal-api-u9al.onrender.com） |
| ローカルバックエンド | **uvicorn常時稼働（port 8734）** |
| DB | Neon PostgreSQL稼働中 |
| 自動売買 | **実資金モード（dryRun=false）** |
| 証券口座総資産 | **¥105,344（2026-07-03 証券会社サイト確認）** |
| 証券口座内訳 | 現金(信用保証金)¥53,207 + 未決済¥6,325 + 保有株¥14,620 + 投信¥31,192 |
| 保有ポジション | 9432(NTT) 100株 @¥146 現値¥146.1 **+¥10(+0.1%)** / 8918(ランド) 1株 @¥63 現値¥10 含み損-¥53 |
| 実資金確定損益 | **+¥4,360**（売却完了9件、手数料¥0） |
| 投信積立 | **全6本売却注文済・積立中止済**（7/9現金化予定、約¥31,000戻り） |
| 自動売買ユニバース | **59銘柄（Phase 45 で 50→59 拡張、全 enabled=true）** |
| 全ページ | 11ページ完了 |
| 全API | 45エンドポイント完了（+1: brokerage/health） |
| 運用ルール | **平日 11:20 までに kabu STATION 手動ログイン**（11:30初回スケジュール） |
| 信用取引 | 口座開設済みだが**最低保証金30万円未達**のため利用不可 |

### 実資金運用状況（2026-07-04）
- 運用開始から約2ヶ月経過。売却完了9件、確定損益 **+¥4,360**
- ただし初回の6753シャープ(+¥4,380)を除くと残り8件は合計 **-¥20**（ほぼトントン）
- 原因分析: NTTを繰り返し売買するも、含み損の状態でシグナル売りが発動し小さな損失を繰り返していた
- **Phase 55 で対策**: 含み損時のシグナル売りを抑制、ATR損切り/-5%固定損切りに到達するまでホールドする制御を追加
- 含み損益合計 **-¥43**（9432 +¥10 / 8918 -¥53）
- **元手10万に対し総資産¥105,344（+¥5,344 / +5.3%）**
- 利確設定は 5%→4%→8%→**10%**（現在値）と試行。少額元手では「伸ばして1回」が効率的と判明
- 8918(ランド) 1株は単元未満株（取得@¥63→現値¥10）。auto-trade は単元(100株)単位発注のためツールから売却不可・要手動S株売却（損益僅少のため放置可）

### 投信積立問題の解決（2026-07-04）
- **原因特定**: kabu STATION APIからは投資信託が見えないため、システム上は現金が約3万円「消えた」ように見えていた
- **真因**: 口座開設時に設定された投信積立（毎月25日・6本・合計¥15,000/月）が自動実行されていた
- **対応**: 投信6本を全額売却注文（7/6約定→7/9現金化）、積立プラン6本を全て中止
- **投信内訳**（売却注文済）:
  - eMAXIS Slim 先進国株式インデックス: ¥13,995（+¥193）
  - たわらノーロード 国内リート: ¥9,756（+¥154）
  - eMAXIS Slim 国内株式(TOPIX): ¥3,401（+¥98）
  - eMAXIS Slim 国内債券インデックス: ¥2,232（-¥19）
  - ニッセイ世界リートオープン: ¥908（+¥3）
  - 三菱UFJグローバル・ボンド・オープン: ¥897（-¥2）
- **7/9以降の見込み**: 現金約¥90,000 + NTT 100株で自動売買の買付余力が大幅改善
...
| maxOpenPositions | 3（5/30に 1→3 へ変更） |
| orderType | market（成行） |
| takeProfitPercent | **10%**（5%→4%→8%→10%と試行、少額元手では伸ばす方が効率的） |
| stopLossPercent | -5%（ATRクランプ適用済み、Phase 50） |
| シグナル売り | **含み損時は抑制**（Phase 55、損切りラインまでホールド） |
| tradingMode | cash（信用は保証金30万円未達のため利用不可）
...
#### 5/25 実況（手動ログイン未実施で全枠 401＝4日連続スキップ）
- ザラ場 09:30〜15:30 の全枠で `kabu STATION接続失敗: 認証エラー: 401` → resultStatus=failed が6件、skipped 3件、計9件
- 原因: 5/25（月）にユーザーが kabu STATION への手動ログインを実施しなかった（運用ルール「11:20までに手動ログイン」未遵守）
- Phase 44 の user-systemd 停止対処自体は機能していた（バックエンドは正常稼働、401はあくまで kabu 側未ログイン）
- diag-market-test は **4日連続未発火**（5/19・5/20・5/21・5/22・5/25）

#### 5/26 実況（接続復旧／約定ゼロは構造要因に切り替わる＝Phase 45 着手）
- 10:00 ユーザー手動ログイン → `/api/brokerage/health` で `status=connected`、`consecutiveFailures=0`、最終成功 14:31:43 を確認
- ザラ場枠ログ **252件すべて skipped、failed 0件**（kabu 認証は終日安定）
- 強度2 buy が **3289（東急不動産HD）で2回発火**（10:02・10:32）→ いずれも「予算不足で購入数量0」でスキップ。1単元 ¥131,750 vs 実残高 ¥85,187 で約 ¥46k 不足
- 既存ユニバース50銘柄のうち、現残高 ¥85,187 で1単元購入可能なのは **6銘柄のみ**（9432 NTT, 9434 ソフトバンク, 4689 LINEヤフー, 5401 日本製鉄, 9501 東電HD, 4755 楽天）
- 残りの44銘柄は strength=2 が発火しても買えない構造 → **Phase 45 で低単価9銘柄をユニバースに追加**（後述）
...
- [ ] Phase 41（調査中）: Code=100378 発注拒否の根本対処（Exchange値の動的切替 / 時間外PTS発注対応 / ザラ場中失敗の真因特定）— 5/20は検証材料取れず（WSLダウン+リスク拒否で発注不発）。5/19・5/20・5/21・5/22・5/25 と diag-market-test 5日連続未発火
- [x] Phase 42: WSL自動起動タスク化（KabuSignalAutoStart: onLogon + 平日09:00）
- [x] Phase 43: WSL2 VM 永続化（.wslconfig vmIdleTimeout=-1） — 12時間51分連続稼働で機能確認
- [x] Phase 44: user-systemd 重複の停止＋wsl-drill リトライ窓5分拡張（未コミット）
- [x] Phase 45: auto-trade ユニバース低単価拡張（50→59銘柄、未コミット）
...
## Phase 45: auto-trade ユニバース低単価拡張（2026-05-26）

### 背景
Phase 39（実残高ベース数量クランプ）と Phase 44（user-systemd 重複停止）を経て、5/26 10:00 の手動ログイン後はバックエンド・kabu接続ともに終日安定。にもかかわらず約定はゼロが継続。本日のログ分析で「**実取引ゼロの真因は接続障害ではなく予算ミスマッチ**」と判明した。

### 当日確認したログ実態（2026-05-26、252件）
| 区分 | 件数 |
|---|---:|
| skipped（hold シグナル＝様子見） | 102 |
| skipped（buy 強度1 < 2） | 66 |
| skipped（sell 強度1 < 2） | 26 |
| skipped（シグナルデータなし） | 4 |
| **skipped（予算不足で購入数量0）** | **2** |
| failed | **0** |
| risk_blocked | 0 |

- 強度2 buy が **2回発火**（いずれも 3289 東急不動産HD、10:02 / 10:32）。1単元 ¥131,750 vs 残高 ¥85,187 で約 ¥46k 不足
- 過去9日(5/18〜5/26)累計500件でも `executed=True` は **0件**、failed 20件は5/18〜5/25 の kabu 401/接続不可・500応答（全て当日中に解消済み）

### 既存ユニバース50銘柄の予算適合性調査（5/26時点）
予算 ¥85,187 で1単元購入可能（株価 ≤ ¥851）なのは **6銘柄のみ**:
| code | 銘柄 | 株価 | 1単元 | 現シグナル |
|---|---|---:|---:|---|
| 9432 | NTT | ¥150.0 | ¥15,000 | hold |
| 9434 | ソフトバンク | ¥216.0 | ¥21,600 | hold |
| 4689 | LINEヤフー | ¥402.4 | ¥40,239 | buy str=1 |
| 5401 | 日本製鉄 | ¥557.4 | ¥55,740 | hold |
| 9501 | 東京電力HD | ¥576.3 | ¥57,629 | hold |
| 4755 | 楽天グループ | ¥755.3 | ¥75,529 | sell str=1 |

→ 残り44銘柄は強度2 buy が出ても買えない構造で、これが**約定ゼロ継続の主因**。

### 対処（5/26 セッション内で適用、未コミット）
1. yfinance（backend venv の v1.3.0）で候補35銘柄の現在価格を取得
2. ¥85,187 以内で1単元購入可能・出来高そこそこのものから**9銘柄**を採用
3. `POST /api/stocks {"code":"XXXX"}` で銘柄をDB登録
4. `PUT /api/auto-trade/stocks/{code} {"enabled":true}` で auto-trade 対象に有効化

### 追加した9銘柄
| code | 銘柄 | 株価 | 1単元 | 出来高 | セクター |
|---|---|---:|---:|---:|---|
| 7211 | 三菱自動車 | ¥400.2 | ¥40,020 | 1,238万 | 自動車 |
| 6753 | シャープ | ¥639.6 | ¥63,959 | 287万 | 電機 |
| 8410 | セブン銀行 | ¥268.6 | ¥26,860 | 278万 | 銀行 |
| 3861 | 王子HD | ¥772.2 | ¥77,220 | 264万 | 製紙 |
| 2127 | 日本M&Aセンター | ¥658.2 | ¥65,820 | 141万 | 金融サービス |
| 5563 | 新日本電工 | ¥528.0 | ¥52,800 | 129万 | 金属 |
| 9831 | ヤマダHD | ¥610.7 | ¥61,070 | 179万 | 家電量販 |
| 2353 | 日本駐車場開発 | ¥243.0 | ¥24,300 | 90万 | 不動産サービス |
| 2317 | システナ | ¥420.0 | ¥42,000 | 51万 | ITサービス |

候補に挙げた4593 ヘリオス（バイオ・¥313）は投機色強いとの判断でユーザー指示により除外。

### 検証結果
- 9銘柄すべて `POST /api/stocks` で正常登録（yfinance 経由で社名・現在価格・初期シグナル付き）
- `PUT /api/auto-trade/stocks/{code}` で全件 `enabled=true` を確認
- ユニバース合計: **50 → 59 銘柄、enabled も 59 件**

### 評価
1. **Phase 39 の予算ガードは想定通り動作**: 5/26 の 3289 で2回連続「予算不足」スキップが発火、kabu sendorder への 500 誘発リクエストはゼロ
2. **約定ゼロは Phase 45 で予算サイドの構造要因に対処**: 9銘柄追加により、強度2 buy が¥85k圏内銘柄でも発火可能な状態を確保
3. **設定 `minSignalStrength=2` は据え置き**: 閾値を1に下げる案も議論したが「黒字データ未取得（過去バックテスト1件は USD_JPY データなしで failed）」のため見送り

### 残課題
- 月曜（5/27）以降のザラ場で、追加9銘柄に強度2 buy が発火し約定に至るかの実地観察
- 約定が発生した場合、Phase 41（Code=100378 発注拒否）の検証材料として kabu sendorder レスポンスを取得できる
- 残高が ¥100,187（5/19確認）→ ¥85,187（5/26確認）へ ¥15,000 減少した経緯不明（実弾発注は0件、transactionsも空。テスト発注時の手数料/前回保有売却等の可能性）。実害なしのため追跡優先度は低
- バックテスト基盤の整備（過去データで `minSignalStrength=1` vs `2` の収益比較ができる状態にする）は未着手

### 変更ファイル
- なし（コード変更ゼロ、API経由のDB操作のみ）
- DB 影響: `stocks` テーブルに9行 INSERT、`auto_trade_stocks` に9行 INSERT（enabled=true）

## Phase 46: minSignalStrength=1 への引き下げ＋phase25マイグレーション撤回（2026-05-27）

### 背景
Phase 45 でユニバースを 50→59 銘柄に拡張し、低単価で1単元購入可能な銘柄を9件追加したものの、5/27 ザラ場の結果は約定 **0 件**。kabu STATION 接続は OK（12:30〜15:00 の 7サイクルすべて 200/トークン取得・残高取得 ¥85,187 まで成功）、それでもシグナル発火がゼロという結果になった。

### 当日確認したサイクル実態（2026-05-27 server.log）
| 時刻 | 状態 |
|---|---|
| 09:30 | サイクル発火痕跡なし（cron未登録 or 別刻み） |
| 10:30 | サイクル発火痕跡なし |
| 11:30 | 「昼休み中」と判定されスキップ |
| 12:30 | 接続OK・残高 ¥85,187・Trades today: 0 |
| 13:00 / 13:30 / 14:00 / 14:30 / 15:00 | いずれも実行成功・Trades today: 0 |

`/api/brokerage/health` → `connected / consecutiveFailures: 0 / lastSuccessAt 2026-05-27T15:02:04`。kabu 側の障害ではなく、シグナル基準で全件足切りされた状態。

### 約定ゼロの構造要因（5/27時点）
全58銘柄の signalStrength 分布:
| strength | 件数 | うち buy |
|---|---:|---:|
| 2 | 5 | **3** |
| 1 | 31 | 20+ |
...
strength=2 buy の 3 銘柄（4063 信越化学 / 9433 KDDI / 9766 コナミ）はいずれも単元金額が ¥40〜50万円台で、残高 ¥85,187 では **1株も買えない**。Phase 45 で追加した低単価銘柄（8410 セブン銀行・3861 王子HD・2353 日本駐車場開発など）はすべて strength=1 で、現行閾値（2）ではエントリー対象外。**Phase 45 の効果が現閾値では発現しない**ことが確定。

### Phase 25 マイグレーションの巻き戻し問題
SCOPE_PROGRESS.md 現行設定表（L47）は `minSignalStrength | 1` を正としているが、起動時の API は **2** を返していた。原因は `backend/src/main.py:386-389` の Phase 25 マイグレーションが毎回 `1 → 2` に上書きしていたため。本日 2026-05-27 16:09 の起動ログでも該当行が実行されており、「Phase 26 で起動時上書きバグ修正済み」と記載があったにもかかわらず、Phase 25 の閾値引き上げ自体は撤回されていなかった。

### 対処（5/27 セッション内で適用、未コミット）
1. `PUT /api/auto-trade/config` で `minSignalStrength: 2 → 1` を DB に反映
2. `backend/src/main.py:386-389` の Phase 25 マイグレーション配列から minSignalStrength 行を削除（次回再起動以降の巻き戻しを止める）。`maxOpenPositions` の Phase 25 行は残置（noop化済み）

### 変更ファイル
- `backend/src/services/auto_trade_service.py` — 変更なし
- `backend/src/main.py` — Phase 25 マイグレーション配列から minSignalStrength 1→2 行を削除（why コメント追加）
- DB 影響: `auto_trade_config.minSignalStrength: 2 → 1`

### 期待効果
- buy 候補 strength≥1: 3 → 23 銘柄（Phase 45 で追加した低単価銘柄も対象に含まれる）
- 残高 ¥85,187 で 100株単元購入が物理的に可能な buy 候補が複数出現:
  - 8410 セブン銀行 RSI=48 / 100株 ¥26,860
  - 3861 王子HD RSI=35 / 100株 ¥77,220
  - 2353 日本駐車場開発 RSI=37 / 100株 ¥24,300
  - 7211 三菱自動車 / 100株 ¥40,020 など

### Phase 45 #3 の判断撤回理由
Phase 45 評価 #3 では「黒字データ未取得のため minSignalStrength=2 を据え置き」と記載したが、5/27 検証で「**現残高では strength=2 buy が物理的に買えない**」ことが確定。バックテスト基盤の整備を待つよりも、低単価ユニバース＋strength=1 で1件でも実約定を取りに行く方が情報価値が高いと判断。

### 残課題
- 5/28 朝のザラ場で実際に約定が発生するかの実地観察
- 09:30 / 10:30 サイクルが server.log に存在しない件の調査（cron 登録の現状確認）
...
## Phase 51: 段階的利確の部分売却数量を単元(100株)単位に丸め（2026-06-03）

### 背景
段階的利確（保有の一部を売却する処理）で算出した売却数量が単元(100株)未満や端数になり、kabu STATION API が **Code 1002**（数量不正）で発注を拒否していた。

### 対処
- 部分売却数量を **100株単位に丸め**てから発注するよう修正
- 丸め後に 0 株となるケースは売却スキップ
...
## Phase 52: 保有銘柄照会で数量0の残骸建玉を除外・None安全化（2026-06-03）

### 背景
売り切った後に kabu /positions が `LeavesQty=0` の残骸建玉を返し、その `ProfitLoss` が `None` のためレスポンス検証（float 必須）が落ち、`/api/brokerage/positions` が **500エラー**になっていた。

### 対処
- `LeavesQty=0` の建玉は保有銘柄ではないので除外
- `averagePrice`/`currentPrice`/`profitLoss` を `None → 0` フォールバックで安全化
...
## Phase 53: 保有銘柄の平均取得単価を正しく取得（2026-06-08）

### 背景
`/api/brokerage/positions` の `averagePrice` が全銘柄で **常に 0** を返していた。kabu STATION API `/positions` のレスポンスに `AveragePrice` というキーは**存在せず**、取得単価は **`Price`** フィールドで返るため、`pos.get('AveragePrice')` が常に `None → 0` にフォールバックしていた。

### 対処
- `averagePrice` の取得元を `pos.get('AveragePrice')` → **`pos.get('Price')`** に修正
- 再起動・実 API で検証済み（2353→¥242 / 9432→¥147 / 8918→¥63 を正しく返す）

### 変更ファイル
- `backend/src/services/brokerage_service.py`

### 反映
- 2026-06-08 16:34 にバックエンド（uvicorn, port 8734）を再起動し反映済み。

---

## Phase 55: シグナル売り抑制 — 含み損時の早期損切り防止（2026-07-02）

### 背景
- 6/5〜7/2の売買実績を分析した結果、初回シャープ(+¥4,380)以外の8件は合計-¥20とほぼ利益なし
- 原因: NTTを146円で買い→MACDデッドクロス等の売りシグナルで143〜145円で損切り→再度買い直し、を繰り返すパターン
- ATR損切り(-2×ATR)や固定損切り(-5%)に到達していないのに、シグナル売りで中途半端な損失を確定していた

### 対処
- `auto_trade_service.py` のイグジット未発動→シグナル売り処理に条件を追加
- **含み損（gain_pct < 0）のときはシグナル売りを抑制**し、ATR損切り/-5%固定損切りに到達するまでホールド
- 含み益のときは従来通りシグナル売りを許可（利確機会は維持）
- 抑制時はログに「シグナル売り抑制 — 含み損 X%、損切りライン到達まで保有継続」を出力

### 変更ファイル
- `backend/src/services/auto_trade_service.py`（シグナル売り抑制ロジック追加）

### 期待効果
- 小さな損失の繰り返し（-160円、-330円等）を防止
- 利確ライン（+10%/ATR段階利確）まで保有を継続できる確率が上がる
- 損切りは-5%/ATRラインで従来通り機能するため、大損リスクは変わらない

---

## Phase 54: 元手の月次トラッキング基盤を構築（2026-06-11）

### 背景
ユーザーの目標は「**追加投資なしで月10万円の継続利益**」。ただし元手は現金約¥5万・現物のみで、月10万は数学的に到達不能。話し合いの結果、当面は「**利益を引き出さず再投資して元手を複利で育てる**」中間目標に置き換え、**月1回 元手の増減を報告**する運用で合意。

### 対処（記録の自動化）
- `scripts/equity-snapshot.sh` を新規作成。元手（= 現金 `cashBalance` + 保有評価額）を CSV `~/.kabu-signal/equity-history.csv`（`date,cash,holdings,total`）に追記。kabu Station 停止中（残高取得不可）は **記録せずスキップ**し、誤データを残さない。
- systemd `--user` タイマー `kabu-equity-snapshot.timer` を追加。**平日15:05（引け直後・kabu稼働中）** に実行、`Persistent=true` で逃した分は次回起動時に1回補完。
- `scripts/equity-report.sh` を新規作成。記録開始・今月初め・最新の元手と増減を集計表示（月次報告用）。
- 初回記録は **2026-06-12 15:05** から。クラウド/Cron はローカル kabu に届かないため自動 push は不可 → 月初の会話時に `equity-report.sh` を実行して報告する運用。

### 変更ファイル
- `scripts/equity-snapshot.sh`（新規）
- `scripts/equity-report.sh`（新規）
- `~/.config/systemd/user/kabu-equity-snapshot.service` / `.timer`（新規）

### 運用メモ
- cloudflared トンネルが 6/11 21:05〜停止していたため再起動（`systemctl --user restart kabu-tunnel.service`）。発行URL `flows-negotiations-norman-rider.trycloudflare.com` を Render に再登録済み。以前頻発した起動15〜28秒での即死は再発せず安定。

---

## 実資金収支の現状サマリー（2026-06-11 時点）

- **累計実現損益: +¥4,735**（6753 +¥4,110 / 9432 +¥435 / 2353 +¥190）
- **本日の約定: 2件**
  - 12:32 売り 9432(NTT) 100株 @¥150.6 → 6/5 取得@¥146.25 を利確 **+¥435**
  - 12:32 売り 2353(日本駐車場開発) 100株 @¥243.4 → 6/5 取得@¥241.5 を利確 **+¥190**
  - 14:02 買い 9432(NTT) 100株 @¥150.4（利確後に買い直し・現在保有）
- **現在の保有: 9432(NTT) 100株**、現値¥150.5（6/11終値）で含み益ほぼ建値（DB平均¥148.35 基準で +¥215 / +1.45%）
- 15:02 の最終サイクルは複数の買い/売りシグナルが出たが **全て数量0でスキップ**（資金頭打ち・リスク制限）。最終サイクルまで全完走
- kabu接続は日中正常（最終成功15:02）、引け後はkabu Station停止のため残高/保有照会は翌営業日に復帰
- **目標と方針**: 月10万は長期ゴールとして保持。当面は元手（現金¥5万規模）を引き出さず複利で育てる。設定は据え置き（利確+4%/損切-5%/現物、minSignalStrength=1、maxTradesPerDay=15）

---

## 実資金収支の現状サマリー（2026-06-08 時点）

- **実現損益: +¥4,110**（6753 シャープ 100株を 6/5 @¥663.7 で利確、取得@¥622.6 ← **初の売却約定・初の確定利益**）
- 6/5 に利確枠で 9432(NTT) @¥146.25 / 2353(日本駐車場開発) @¥241.5 を新規約定（低位株分散方針が実約定に到達）
- **含み損益: +¥258**（2353 +¥200 / 9432 +¥110 / 8918(ランド,1株) -¥52）
- 口座評価額: 現金¥50,522 + 保有株¥39,151 ≈ **¥89,673**（元本¥100,187 から -¥10,514、ただし確定損益は黒字転換）
- 6/8 場中: 新規約定なし（買いシグナルは既保有でskip、売りシグナルは数量0でskip）
- 課題: 8918 単元未満株1株の手仕舞い（要手動S株売却・損益僅少）、averagePrice バグは Phase 53 で解消

---

## 実資金収支の現状サマリー（2026-06-02 時点）

- **実現損益: ¥0**（売却約定が一度も無い）
- 唯一の成功約定: 6753（シャープ）100株 @¥622.6（filled確認済み、`brokerage_order_id=20260529A02N72098838`）。他94件は Code=100378 で `failed`（Phase 48 で修正済み）
- **含み損益: +¥2,890**（6753 現値¥651.9、+4.7%）/ 8918(ランド) -¥53
- 口座評価額: ¥88,127（現金¥22,927 + 保有株¥65,200）← 元本¥100,187 から -¥12,060
- **利確ライン変更: 10%→5%（6/2）** → 6753の利確ライン¥653.7に現値が接近中
- 信用取引: 口座開設済みだが最低保証金30万円に対し保証金¥73,311で**利用不可**。追加入金なしの方針
- 戦略: 現物のみ、利確5%/損切-5%で低位株を回転させる方針に転換
- 次の一手: 6753利確後、現金¥88k で低位株（NTT/SB/日本駐車場等、100株¥1.5〜2万）を複数ポジションで回転
