...
## 現在のステータス（2026-06-08 時点）

| 項目 | 状態 |
|------|------|
| 最終フェーズ | **Phase 53（保有銘柄の平均取得単価を正しく取得）** |
| 最終コミット | 0238115（2026-06-08） |
| フロントエンド | Vercel稼働中（kabu-signal-navi.vercel.app） |
| バックエンド | Render稼働中（stock-signal-api-u9al.onrender.com） |
| ローカルバックエンド | **uvicorn常時稼働（port 8734、6/8 16:34 再起動・Phase53反映）** |
| DB | Neon PostgreSQL稼働中 |
| 自動売買 | **実資金モード（dryRun=false）** |
| 証券口座残高 | **現金¥50,522 + 保有株評価¥39,151 ≈ ¥89,673（2026-06-08 API確認）** |
| 保有ポジション | 2353(日本駐車場開発) 100株 @¥242 現値¥244 **+¥200(+0.8%)** / 9432(NTT) 100株 @¥146.3 現値¥147.4 **+¥110(+0.8%)** / 8918(ランド) 1株 @¥63 現値¥11 含み損-¥52 |
| 実資金確定損益 | **+¥4,110**（6753 を 6/5 @¥663.7 利確、取得@¥622.6 ← 初の売却約定） |
...
| 信用取引 | 口座開設済みだが**最低保証金30万円未達**(現保証金¥73,311)のため利用不可 |

### 実資金運用状況（2026-06-08）
- kabu STATION API接続確認済み（connected、最終成功 6/8 15:02）
- **6753（シャープ）を 6/5 に @¥663.7 で利確売り（取得@¥622.6）→ 初の実現損益 +¥4,110 を確定**
- 同 6/5、利確で空いた現金枠で **9432(NTT) @¥146.25 / 2353(日本駐車場開発) @¥241.5 を新規約定**（低位株への分散方針が実約定に到達）
- 6/8 場中（15:02〜15:32）: 新規約定なし。2353 買いシグナル(強度2)→既保有でskip、2317/9831 売りシグナル→保有数量0でskip、15:32 取引時間外で正常終了
- 実資金収支: **確定損益 +¥4,110 / 含み損益 +¥258**（2353 +¥200 / 9432 +¥110 / 8918 -¥52）
- 8918(ランド) 1株は単元未満株（取得@¥63→現値¥11）。auto-trade は単元(100株)単位発注のためツールから売却不可・要手動S株売却（損益僅少のため放置可）
...
| orderType | market（成行） |
| takeProfitPercent | **5%**（6/2に 10%→5% へ変更、回転重視） |
