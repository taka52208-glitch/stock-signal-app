...
## 現在のステータス（2026-06-03 時点）

| 項目 | 状態 |
|------|------|
| 最終フェーズ | **Phase 50（損切りクランプ）** |
| 最終コミット | 3120402（2026-05-30） |
| フロントエンド | Vercel稼働中（kabu-signal-navi.vercel.app） |
| バックエンド | Render稼働中（stock-signal-api-u9al.onrender.com） |
| ローカルバックエンド | **uvicorn常時稼働（port 8734、5/30〜連続稼働中）** |
| DB | Neon PostgreSQL稼働中 |
| 自動売買 | **実資金モード（dryRun=false）** |
| 証券口座残高 | **現金¥22,927 + 保有株評価¥66,860 ≈ ¥89,787（2026-06-03 API確認）** |
| 保有ポジション | 6753(シャープ) 100株 @¥622.6 現値¥668.5 **含み益+¥4,550(+7.3%)** / 8918(ランド) 1株 含み損-¥53 |
| 実資金確定損益 | ¥0（売却約定なし） |
...
### 実資金運用状況（2026-06-03）
- kabu STATION API接続確認済み（connected）
- 6753（シャープ）100株が唯一の成功約定（5/29受理、filled確認済み）
- 買いシグナル発生中だが現金¥22,927では100株単位で予算不足→スキップ
- **6753が利確ライン¥653.7（+5%）を突破、現値¥668.5（+7.3%）→ 本日の自動売買サイクルで利確売りの可能性。約定すれば+約¥4,550が確定損益化**
- 実資金収支: 確定損益¥0 / 含み損益**+¥4,497**（6753 +¥4,550 / 8918 -¥53）
