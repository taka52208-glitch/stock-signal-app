...
| 証券口座残高 | **現金¥22,927 + 保有株評価¥65,200 ≈ ¥88,127（2026-06-02 API確認）** |
| 保有ポジション | 6753(シャープ) 100株 @¥622.6 現値¥651.9 **含み益+¥2,890(+4.7%)** / 8918(ランド) 1株 含み損-¥53 |
| 実資金確定損益 | ¥0（売却約定なし） |
...
### 実資金運用状況（2026-06-02）
- kabu STATION API接続確認済み（connected、最終成功 6/2 15:02）
- 6/2 手動ログイン実施済み、自動売買サイクル正常稼働
- 6753（シャープ）100株が唯一の成功約定（5/29受理、filled確認済み）
- 買いシグナル発生中（2317, 2353等）だが現金¥22,927では100株単位で予算不足→スキップ
- **6753の利確ライン¥653.7（+5%）に現値¥651.9で接近中 → 早ければ翌営業日に利確の可能性**
