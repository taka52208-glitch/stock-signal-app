...
- [x] 自動売買設定ページ（ドライランモード・銘柄別設定・実行ログ・通常API経由）
...
- [x] 自動売買サービス（シグナル連動・ドライラン・保守的フィルター・全ケースログ記録）
- [x] 定期更新スケジューラ（09:30 / 12:30 / 15:30 + アラート・自動売買連動 + watchdog復帰対策）
...
| バックエンドプロセス監視 | 中 | start.shでuvicornが落ちても再起動されない。子プロセス監視ループの追加が必要 |
